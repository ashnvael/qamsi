...
class CGPUCB(object):
    def __init__(self, actions, contexts, sample_from_environment, kernel, delta=0.80):
        ...
        self.mu = np.array([0.0 for _ in range(self.input_space.shape[0])])
        self.sigma = np.array([0.5 for _ in range(self.input_space.shape[0])])

        if kernel is None:
            # default kernel
            kernel = GPy.kern.RBF(
                input_dim=self.input_space[0].size, variance=1.0, lengthscale=1.0
            )
    ...
    def cgp_ucb_rule(self, context_index):
        ...
        return lower_bound_on_actions + np.argmax(
            ...
            + self.sigma[lower_bound_on_actions:upper_bound_on_actions]
            * np.sqrt(self.beta)
        )
    ...
    def learn(self, context_index):
        ...
        self.beta = self.optimal_beta_selection(
            self.round, self.input_space_size, self.delta
        )
        ...
        self.gp = GPy.models.GPRegression(
            np.array(self.X), np.array(self.Y), self.kernel
        )
    ...
    def plot_environment_and_mean(self):
        ...
        ax.plot_wireframe(
            self.input_mesh[0],
            self.input_mesh[1],
            self.mu.reshape(self.input_mesh[0].shape),
            alpha=0.6,
            color="g",
            label="mean",
        )
        ...
        ax.plot_wireframe(
            self.input_mesh[0],
            self.input_mesh[1],
            np.array(self.sample_from_environment(self.input_mesh)).reshape(
                self.input_mesh[0].shape
            ),
            alpha=0.5,
            color="b",
            label="environment",
    ...
    def plot_slices(self, fixed_dimension, slices, density=False):
        ...
        for i, y in zip(range(number_of_slices), slices):
            self.gp.plot(
                ...
                fixed_inputs=[(fixed_dimension, y)],
                row=(i + 1),
                plot_data=False,
                title="slice at %s=%f" % (fixed_dim_name, y),
                xlabel=xlabel,
                ylabel="reward",
            )
        return
...
if __name__ == "__main__":
    ...
    kernel1 = GPy.kern.RBF(input_dim=1, variance=1.0, lengthscale=1.0, active_dims=[0])
    # works on the second dim. of input_space, index=1
    kernel2 = GPy.kern.RBF(input_dim=1, variance=1.0, lengthscale=1.0, active_dims=[1])
    ...
    agent = CGPUCB(
        actions=actions,
        contexts=contexts,
        sample_from_environment=environment.sample_noisy,
        kernel=kernel,
    )
    ...
    for i in range(rounds):
        ...
        best_strategy_rewards.append(
            environment.get_best_reward_in_context(context_index)
        )
    ...
    agent.builtin_plot(
        projection="2d", title="GPy Contour Plot", xlabel="Actions", ylabel="Contexts"
    )
